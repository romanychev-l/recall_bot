...
message-onboarded_zero = Начинаем с самых частотных слов. Готово!
message-onboarded_skip = Пропускаю первые { $n } слов. Готово!
message-ask_skip_n = Сколько слов пропустить (от 0 до 100000)? Отправь число.
...
message-stats_summary =
    ...
    Выучено всего: { $graduated }
    Точность за 7 дней: { $accuracy }
    Серия дней подряд: { $streak }

    Прогресс по CEFR:
    { $cefr }

button-start_zero = С нуля (с самых частотных)
button-start_skip = Я уже знаю N слов
